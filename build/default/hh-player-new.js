...
return polymerElementBase._addTemplatePropertyEffect.call(this,templateInfo,prop,effect)}}return PolymerElement}),updateStyles=function(props){if(window.ShadyCSS){window.ShadyCSS.styleDocument(props)}};var elementMixin={version:version,ElementMixin:ElementMixin,updateStyles:updateStyles};class LiteralString{constructor(string){/** @type {string} */this.value=string.toString()}/**
   ...
   */const html=function html(strings,...values){const template=/** @type {!HTMLTemplateElement} */document.createElement("template");template.innerHTML=values.reduce((acc,v,idx)=>acc+htmlValue(v)+strings[idx+1],strings[0]);return template},htmlLiteral=function(strings,...values){return new LiteralString(values.reduce((acc,v,idx)=>acc+literalValue(v)+strings[idx+1],strings[0]))};/**
    ...
    */var htmlTag={html:html,htmlLiteral:htmlLiteral};const PolymerElement=ElementMixin(HTMLElement);var polymerElement={version:version,PolymerElement:PolymerElement,html:html};/**
   @license
   Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
   This code may only be used under the BSD style license found at http://polymer.github.io/LICENSE.txt
   The complete set of authors may be found at http://polymer.github.io/AUTHORS.txt
...
`,is:"iron-icon",properties:{/**
     ...
     */src:{type:String},/**
     * @type {!IronMeta}
     */_meta:{value:Base.create("iron-meta",{type:"iconset"})}},observers:["_updateIcon(_meta, isAttached)","_updateIcon(theme, isAttached)","_srcChanged(src, isAttached)","_iconChanged(icon, isAttached)"],_DEFAULT_ICONSET:"icons",_iconChanged:function(icon){var parts=(icon||"").split(":");this._iconName=parts.pop();this._iconsetName=parts.pop()||this._DEFAULT_ICONSET;this._updateIcon()},_srcChanged:function(src){this._updateIcon()},_usesIconset:function(){return this.icon||!this.src},/** @suppress {visibility} */_updateIcon:function(){if(this._usesIconset()){if(this._img&&this._img.parentNode){dom(this.root).removeChild(this._img)}if(""===this._iconName){if(this._iconset){this._iconset.removeIcon(this)}}else if(this._iconsetName&&this._meta){this._iconset=/** @type {?Polymer.Iconset} */this._meta.byKey(this._iconsetName);if(this._iconset){this._iconset.applyIcon(this,this._iconName,this.theme);this.unlisten(window,"iron-iconset-added","_updateIcon")}else{this.listen(window,"iron-iconset-added","_updateIcon")}}}else{if(this._iconset){this._iconset.removeIcon(this)}if(!this._img){this._img=document.createElement("img");this._img.style.width="100%";this._img.style.height="100%";this._img.draggable=!1}this._img.src=this.src;dom(this.root).appendChild(this._img)}}});const template$1=html`
<custom-style>
  <style is="custom-style">
    html {
...
</custom-style>`;template$2.setAttribute("style","display: none;");document.head.appendChild(template$2.content);const IronControlState={properties:{/**
...
if(event.defaultPrevented){return}}}},_triggerKeyHandler:function(keyCombo,handlerName,keyboardEvent){var detail=Object.create(keyCombo);detail.keyboardEvent=keyboardEvent;var event=new CustomEvent(keyCombo.event,{detail:detail,cancelable:!0});this[handlerName].call(this,event);if(event.defaultPrevented){keyboardEvent.preventDefault()}}};var ironA11yKeysBehavior={IronA11yKeysBehavior:IronA11yKeysBehavior};const IronButtonStateImpl={properties:{/**
...
}}},IronButtonState=[IronA11yKeysBehavior,IronButtonStateImpl];/** @polymerBehavior */var ironButtonState={IronButtonStateImpl:IronButtonStateImpl,IronButtonState:IronButtonState},Utility={distance:function(x1,y1,x2,y2){var xDelta=x1-x2,yDelta=y1-y2;return Math.sqrt(xDelta*xDelta+yDelta*yDelta)},now:window.performance&&window.performance.now?window.performance.now.bind(window.performance):Date.now};/**
    * @param {HTMLElement} element
    * @constructor
...
var domContainer=dom(this._rippleContainer||this),target=dom(optTriggeringEvent).rootTarget;if(domContainer.deepContains(/** @type {Node} */target)){this._ripple.uiDownAction(optTriggeringEvent)}}}},/**
   ...
   */_createRipple:function(){var element=/** @type {!PaperRippleElement} */document.createElement("paper-ripple");return element},_noinkChanged:function(noink){if(this.hasRipple()){this._ripple.noink=noink}}};var paperRippleBehavior={PaperRippleBehavior:PaperRippleBehavior};const PaperInkyFocusBehaviorImpl={observers:["_focusedChanged(receivedFocusFromKeyboard)"],_focusedChanged:function(receivedFocusFromKeyboard){if(receivedFocusFromKeyboard){this.ensureRipple()}if(this.hasRipple()){this._ripple.holdDown=receivedFocusFromKeyboard}},_createRipple:function(){var ripple=PaperRippleBehavior._createRipple();ripple.id="ink";ripple.setAttribute("center","");ripple.classList.add("circle");return ripple}},PaperInkyFocusBehavior=[IronButtonState,IronControlState,PaperRippleBehavior,PaperInkyFocusBehaviorImpl];/** @polymerBehavior */var paperInkyFocusBehavior={PaperInkyFocusBehaviorImpl:PaperInkyFocusBehaviorImpl,PaperInkyFocusBehavior:PaperInkyFocusBehavior};Polymer({is:"paper-icon-button",/** @override */_template:html`
    ...
    <iron-icon id="icon" src="[[src]]" icon="[[icon]]"
               alt$="[[alt]]"></iron-icon>
  `,hostAttributes:{role:"button",tabindex:"0"},behaviors:[PaperInkyFocusBehavior],/** @override */registered:function(){this._template.setAttribute("strip-whitespace","")},properties:{/**
...
if(!label||oldValue==label){this.setAttribute("aria-label",newValue)}}});Polymer({is:"iron-iconset-svg",properties:{/**
...
// shadow-root
svg.style.cssText=cssText;svg.appendChild(content).removeAttribute("id");return svg}return null}});const template$3=html`<iron-iconset-svg name="av" size="24">
<svg><defs>
...
</iron-iconset-svg>`;document.head.appendChild(template$3.content);const IronFitBehavior={properties:{/**
...
document.documentElement.scrollTop=document.body.scrollTop=this.__scrollTop;document.documentElement.scrollLeft=document.body.scrollLeft=this.__scrollLeft}}},composedParent=node=>node.assignedSlot||node.parentNode||node.host,composedContains=(ancestor,descendant)=>{for(let element=descendant;element;element=composedParent(element)){if(element===ancestor){return!0}}return!1},IronOverlayBehavior=[IronFitBehavior,IronResizableBehavior,IronOverlayBehaviorImpl];/**
                                                                                                       ...
                                                                                                       */ /**
                                                                                                           ...
                                                                                                           */ /**
                                                                                                               ...
                                                                                                               */var ironOverlayBehavior={IronOverlayBehaviorImpl:IronOverlayBehaviorImpl,IronOverlayBehavior:IronOverlayBehavior};const NeonAnimatableBehavior={properties:{/**
...
if(!this._readied){return}if(!allowOutsideScroll){this.scrollAction="lock"}else if(!this.scrollAction||"lock"===this.scrollAction){this.scrollAction="refit"}},/**
   ...
   */_applyFocus:function(){var focusTarget=this.focusTarget||this.containedElement;if(focusTarget&&this.opened&&!this.noAutoFocus){focusTarget.focus()}else{IronOverlayBehaviorImpl._applyFocus.apply(this,arguments)}}});const NeonAnimationBehavior={properties:{/**
...
</dom-module>`;document.head.appendChild($_documentContainer$1.content);const IronFormElementBehavior={properties:{/**
...
if(value===void 0&&this.value!==void 0)this.invalid=!this._getValidity(this.value);else this.invalid=!this._getValidity(value);return!this.invalid},/**
   ...
   * @return {boolean} True if `value` is valid.
   */_getValidity:function(value){if(this.hasValidator()){return this._validator.validate(value)}return!0}};var ironValidatableBehavior={get IronValidatableBehaviorMeta(){return IronValidatableBehaviorMeta},IronValidatableBehavior:IronValidatableBehavior};// with the `Polymer` function, so this is only a cache lookup.
...
`,is:"paper-dropdown-menu-light",behaviors:[IronButtonState,IronControlState,PaperRippleBehavior,IronFormElementBehavior,IronValidatableBehavior],properties:{/**
...
if(this.$.input&&this.$.input.textContent!==this.value){this.$.input.textContent=this.value}this._setHasContent(!!this.value)}});class IronSelection{/**
...
</custom-style>`;template$5.setAttribute("style","display: none;");document.head.appendChild(template$5.content);const $_documentContainer$2=document.createElement("template");$_documentContainer$2.setAttribute("style","display: none;");$_documentContainer$2.innerHTML=`<dom-module id="paper-item-shared-styles">
  <template>
    <style>
      :host, .paper-item {
        display: block;
...
</dom-module>`;document.head.appendChild($_documentContainer$2.content);const PaperItemBehaviorImpl={hostAttributes:{role:"option",tabindex:"0"}},PaperItemBehavior=[IronButtonState,IronControlState,PaperItemBehaviorImpl];/** @polymerBehavior */var paperItemBehavior={PaperItemBehaviorImpl:PaperItemBehaviorImpl,PaperItemBehavior:PaperItemBehavior};Polymer({/** @override */_template:html`
    ...
    <slot></slot>
`,is:"paper-item",behaviors:[PaperItemBehavior]});const IronA11yAnnouncer=Polymer({_template:html`
    <style>
      :host {
        display: inline-block;
        position: fixed;
...
`,is:"paper-input-char-counter",behaviors:[PaperInputAddonBehavior],properties:{_charCounterStr:{type:String,value:"0"}},/**
   ...
   *     value: The input value.
   *     invalid: True if the input value is invalid.
   */update:function(state){if(!state.inputElement){return}state.value=state.value||"";var counter=state.value.toString().length.toString();if(state.inputElement.hasAttribute("maxlength")){counter+="/"+state.inputElement.getAttribute("maxlength")}this._charCounterStr=counter}});const template$6=html`
<custom-style>
  <style is="custom-style">
    html {
...
if(!!this.inputElement.bindValue){this.$.container._handleValueAndAutoValidate(this.inputElement)}}});const IronRangeBehavior={properties:{/**
...
`;template$7.setAttribute("strip-whitespace","");/**
                                                 ...
                                                 @demo demo/index.html
                                                 */Polymer({_template:template$7,is:"paper-slider",behaviors:[IronA11yKeysBehavior,IronFormElementBehavior,PaperInkyFocusBehavior,IronRangeBehavior],properties:{value:{type:Number,value:0},/**
     ...
     */dragging:{type:Boolean,value:!1,readOnly:!0,notify:!0},transiting:{type:Boolean,value:!1,readOnly:!0},markers:{type:Array,readOnly:!0,value:function(){return[]}}},observers:["_updateKnob(value, min, max, snaps, step)","_valueChanged(value)","_immediateValueChanged(immediateValue)","_updateMarkers(maxMarkers, min, max, snaps)"],hostAttributes:{role:"slider",tabindex:0},/** @type {!Object} */keyBindings:{left:"_leftKey",right:"_rightKey","down pagedown home":"_decrementKey","up pageup end":"_incrementKey"},/** @override */ready:function(){if(this.ignoreBarTouch){setTouchAction(this.$.sliderBar,"auto")}},/**
...
if(receivedFocusFromKeyboard){this._ripple.style.display=""}else{this._ripple.style.display="none"}this._ripple.holdDown=receivedFocusFromKeyboard}}/**
     ...
     */ /**
         ...
         */ /**
             ...
             *
             * @event change
             */});class HhPlayerNew extends PolymerElement{static get template(){return html`
      <style>
        :host {
                display: block;
                width: 400px;
    ...
    `}ready(){super.ready();this.$.fpsLabel.textContent=this.$.fpsRatings.value;this.$.fpsRatings._valueChanged=value=>{this.$.fpsLabel.textContent=value}}static get properties(){return{frameRate:{type:Number,value:0,observer:"_frameRateChanged"},playing:{type:Boolean,value:!0,observer:"_framePlayChanged"},selected:{type:String,value:0,observer:"_viewTypeChanged"},data:{type:Object,observer:"_dataChanged"}}}playingToggle(){this.playing=!this.playing}forwardClick(){this.dispatchEvent(new CustomEvent("frameForwardRewindEvent",{detail:1}))}rewindClick(){this.dispatchEvent(new CustomEvent("frameForwardRewindEvent",{detail:-1}))}_dataChanged(newValue,oldValue){this._initComponent();if(-1<newValue.frameTimeType){this.selected=newValue.frameTimeType}if(newValue.frameTimeVector&&0<newValue.frameTimeVector.length&&""!=newValue.frameTimeVector){}else{this.$.vector.setAttribute("disabled","disabled");this.selected=1;this.$.fps.classList.toggle("active",!0)}this.frameRate=newValue.frameRate;this.playing=newValue.framePlay}_initComponent(){this.selected=0;this.frameRate=0;this.$.fps.classList.toggle("active",!1);this.$.vector.removeAttribute("disabled")}_viewTypeChanged(newValue,oldValue){if(0==newValue){this.$.fps.classList.toggle("active",!1)}if(1==newValue){this.$.fps.classList.toggle("active",!0)}this.playing=!0;this.dispatchEvent(new CustomEvent("frameTimeTypeChangedEvent",{detail:this.selected}))}_frameRateChanged(newValue,oldValue){this.dispatchEvent(new CustomEvent("frameRateChangedEvent",{detail:newValue}));this.playing=!0}_framePlayChanged(newValue,oldValue){this.dispatchEvent(new CustomEvent("framePlayChangedEvent",{detail:newValue}))}}window.customElements.define("hh-player-new",HhPlayerNew);export{applyShim as $applyShim$1,ApplyShim as $applyShimDefault,applyShimUtils as $applyShimUtils,arraySelector as $arraySelector,arraySplice as $arraySplice,async as $async,caseMap$1 as $caseMap,_class as $class,commonRegex as $commonRegex,commonUtils as $commonUtils,cssParse as $cssParse,customStyle as $customStyle,customStyleInterface as $customStyleInterface$1,CustomStyleInterface as $customStyleInterfaceDefault,debounce as $debounce,dirMixin as $dirMixin,documentWait$1 as $documentWait,documentWait as $documentWaitDefault,domBind as $domBind,domIf as $domIf,domModule as $domModule,domRepeat as $domRepeat,elementMixin as $elementMixin,flattenedNodesObserver as $flattenedNodesObserver,flush$2 as $flush,gestureEventListeners as $gestureEventListeners,gestures$1 as $gestures,hideTemplateControls as $hideTemplateControls,htmlTag as $htmlTag,ironA11yAnnouncer as $ironA11yAnnouncer,ironA11yKeysBehavior as $ironA11yKeysBehavior,ironButtonState as $ironButtonState,ironControlState as $ironControlState,ironFitBehavior as $ironFitBehavior,ironFocusablesHelper as $ironFocusablesHelper,ironFormElementBehavior as $ironFormElementBehavior,ironMenuBehavior as $ironMenuBehavior,ironMeta as $ironMeta,ironMultiSelectable as $ironMultiSelectable,ironOverlayBehavior as $ironOverlayBehavior,ironOverlayManager as $ironOverlayManager,ironRangeBehavior as $ironRangeBehavior,ironResizableBehavior as $ironResizableBehavior,ironScrollManager as $ironScrollManager,ironSelectable as $ironSelectable,ironSelection as $ironSelection,ironValidatableBehavior as $ironValidatableBehavior,legacyElementMixin as $legacyElementMixin,mixin as $mixin,mutableData as $mutableData,mutableDataBehavior as $mutableDataBehavior,neonAnimatableBehavior as $neonAnimatableBehavior,neonAnimationBehavior as $neonAnimationBehavior,neonAnimationRunnerBehavior as $neonAnimationRunnerBehavior,paperInkyFocusBehavior as $paperInkyFocusBehavior,paperInputAddonBehavior as $paperInputAddonBehavior,paperInputBehavior as $paperInputBehavior,paperItemBehavior as $paperItemBehavior,paperMenuButton as $paperMenuButton,paperRippleBehavior as $paperRippleBehavior,path as $path,polymer_dom as $polymerDom,polymerElement as $polymerElement,polymerFn as $polymerFn,polymerLegacy as $polymerLegacy,propertiesChanged as $propertiesChanged,propertiesMixin as $propertiesMixin,propertyAccessors as $propertyAccessors,propertyEffects as $propertyEffects,renderStatus as $renderStatus,resolveUrl$1 as $resolveUrl,scopeSubtree$1 as $scopeSubtree,settings as $settings,styleGather as $styleGather,styleSettings as $styleSettings,styleUtil as $styleUtil,telemetry as $telemetry,templateMap$1 as $templateMap,templateMap as $templateMapDefault,templateStamp as $templateStamp,templatize$1 as $templatize,templatizerBehavior as $templatizerBehavior,unscopedStyleHandler as $unscopedStyleHandler,wrap$1 as $wrap,ANIMATION_MATCH,ArraySelector,ArraySelectorMixin,BRACKETED,Base,Class,CustomStyle,CustomStyleInterfaceInterface,CustomStyleProvider,Debouncer,DirMixin,DomApi,DomBind,DomIf,DomModule,DomRepeat,ElementMixin,EventApi,FlattenedNodesObserver,GestureEventListeners,HOST_PREFIX,HOST_SUFFIX,IS_VAR,IronA11yAnnouncer,IronA11yKeysBehavior,IronButtonState,IronButtonStateImpl,IronControlState,IronFitBehavior,IronFocusablesHelper,IronFormElementBehavior,IronMenuBehavior,IronMenuBehaviorImpl,IronMeta,IronMultiSelectableBehavior,IronMultiSelectableBehaviorImpl,IronOverlayBehavior,IronOverlayBehaviorImpl,IronOverlayManager,IronOverlayManagerClass,IronRangeBehavior,IronResizableBehavior,IronSelectableBehavior,IronSelection,IronValidatableBehavior,IronValidatableBehaviorMeta,LegacyElementMixin,MEDIA_MATCH,MIXIN_MATCH,MutableData,MutableDataBehavior,NeonAnimatableBehavior,NeonAnimationBehavior,NeonAnimationRunnerBehavior,NeonAnimationRunnerBehaviorImpl,OptionalMutableData,OptionalMutableDataBehavior,PaperInkyFocusBehavior,PaperInkyFocusBehaviorImpl,PaperInputAddonBehavior,PaperInputBehavior,PaperInputBehaviorImpl,PaperInputHelper,PaperItemBehavior,PaperItemBehaviorImpl,PaperMenuButton,PaperRippleBehavior,Polymer,Polymer as Polymer$1,PolymerElement,PropertiesChanged,PropertiesMixin,PropertyAccessors,PropertyEffects,StyleNode,TemplateInstanceBase,TemplateStamp,Templatizer,VAR_ASSIGN,VAR_CONSUMED,_boundScrollHandler,_composedTreeContains,_getScrollInfo,_getScrollableNodes,_getScrollingNode,_hasCachedLockedElement,_hasCachedUnlockedElement,_lockScrollInteractions,_lockedElementCache,_lockingElements,_scrollInteractionHandler,_shouldPreventScrolling,_unlockScrollInteractions,_unlockedElementCache,add,enqueueDebouncer as addDebouncer,addListener,afterNextRender,allowTemplateFromDomModule,animationFrame,applyCss,applyStyle,applyStylePlaceHolder,beforeNextRender,calculateSplices,camelToDashCase,cancelSyntheticClickEvents,createScopeStyle,cssBuild,cssFromModule,cssFromModuleImports,cssFromModules,cssFromTemplate,currentLockingElement,dashToCamelCase,dedupingMixin,deepTargetFind,detectMixin,disableRuntime,dom,dumpRegistrations,elementHasBuiltCss,elementIsScrollLocked,elementsAreInvalid,enqueueDebouncer,enqueueDebouncer as enqueueDebouncer$1,findMatchingParen,findOriginalTarget,flush$1 as flush,flush$1,flush as flush$2,flushDebouncers,forEachRule,gatherStyleText,gestures,get,getBuildComment,getComputedStyleValue,getCssBuild,getIsExtends,hideElementsGlobally,html,html as html$1,html as html$2,htmlLiteral,idlePeriod,incrementInstanceCount,instanceCount,invalidate,invalidateTemplate,isAncestor,isDeep,isDescendant,isKeyframesSelector,isOptimalCssBuild,isPath,isTargetedBuild,isUnscopedStyle,isValid,isValidating,legacyOptimizations,matches,matchesSelector,microTask,mixinBehaviors,modelForElement,nativeCssVariables,nativeShadow,normalize,parse,passiveTouchGestures,pathFromUrl,prevent,processUnscopedStyle,processVariableAndFallback,pushScrollLock,recognizers,register$1 as register,register as register$1,registrations,remove,removeCustomPropAssignment,removeListener,removeScrollLock,resetMouseCanceller,resolveCss,resolveUrl,root,rootPath,rulesForStyle,sanitizeDOMValue,scopeSubtree,scopingAttribute,set,setAllowTemplateFromDomModule,setCancelSyntheticClickEvents,setElementClassRaw,setLegacyOptimizations,setPassiveTouchGestures,setRootPath,setSanitizeDOMValue,setStrictTemplatePolicy,setSyncInitialRender,setTouchAction,split,splitSelectorList,startValidating,startValidatingTemplate,strictTemplatePolicy,stringify,stylesFromModule,stylesFromModuleImports,stylesFromModules,stylesFromTemplate,syncInitialRender,templateIsValid,templateIsValidating,templatize,timeOut,toCssText,translate,types,updateNativeProperties,updateStyles,useNativeCSSProperties,useNativeCustomElements,useShadow,version,version as version$1,wrap,wrap$2 as wrap$1};